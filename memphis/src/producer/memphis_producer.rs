...
impl MemphisProducer {
    pub(crate) async fn new(station: MemphisStation, mut options: MemphisProducerOptions) -> Result<Self, RequestError> {
    ...
    pub async fn produce(&self, mut message: ComposableMessage) -> Result<(), ProducerError> {
        ...
        message
            .headers
            .insert(MemphisHeaders::MemphisConnectionId, self.station.memphis_client.connection_id.as_str());
        ...
        if let Err(e) = self
            ...
            .publish_with_headers(
                format!("{}.final", get_internal_name(&self.station.options.station_name)),
                message.headers,
                message.payload,
            )
            .await
